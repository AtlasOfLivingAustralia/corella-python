...
def test_individualCount_not_numeric():
    ...
    with pytest.raises(Exception) as e_info:
        corella.set_abundance(dataframe=df)
...
def test_individualCount_renamed():
    ...
    new_df = corella.set_abundance(dataframe=df,individualCount='individuals')
...
def test_individualCount_data_works():
    df = pd.DataFrame({'individualCount': [1,1]})
    new_df = corella.set_abundance(dataframe=df)
