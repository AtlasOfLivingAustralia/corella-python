...
def test_decimalLatitude_not_float():
    df = pd.DataFrame({'decimalLatitude': ['-1.0','-1.0'],'decimalLatitude': [-1.0,-1.0],})
...
def test_coordinatePrecision_coordinateUncertaintyInMeters_not_in_range():
    df = pd.DataFrame({'decimalLatitude': [-1.0,-1.0],'decimalLongitude': [-1.0,-1.0],'coordinatePrecision': [-1.0,-1.0],'coordinateUncertaintyInMeters': [-1.0,-1.0]})
    errors = corella.check_coordinates(dataframe=df,errors=[])
    assert len(errors) == 2
